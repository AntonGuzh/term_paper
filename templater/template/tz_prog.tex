...
\BLOCK{ for item in model.analogs_review: }
\subsubsection*{\VAR{item.name}}
\VAR{item.text}
\BLOCK{ endfor }

\subsection*{Итоги}
\VAR{model.analogs_summary}
...
\BLOCK{ for req in model.func_req: }
\paragraph{\VAR{req.name}}
\begin{enumerate}
    \BLOCK{ for step in req.items: }
    \item \VAR{step}
    \BLOCK{ endfor }
\end{enumerate}
\BLOCK{ endfor }
...
\begin{itemize}
    \BLOCK{ for item in model.non_func_req: }
    \item \VAR{item}
    \BLOCK{ endfor }
\end{itemize}
...
\begin{enumerate}
    \BLOCK{ for item in model.technologies: }
    \item \textit{\VAR{item.name}} --- \VAR{item.text}
    \BLOCK{ endfor }
\end{enumerate}
